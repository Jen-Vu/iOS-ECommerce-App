...
class PrintersViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        allPrinters.append(Printer(name: "HP InkJet", category: "Inkjet", price: 20.00, latitude: 53, longitude: -113, imageName: "Printer", description: text, id: 2))
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "PrinterCell") as! CustomPrinterCell
        
        cell.printerImage.image = #imageLiteral(resourceName: printerCategories[indexPath.row].image)
        cell.nameLabel.text = printerCategories[indexPath.row].name
        cell.priceLabel.text = "$" + String(printerCategories[indexPath.row].price)
        cell.categoryLabel.text = printerCategories[indexPath.row].category
